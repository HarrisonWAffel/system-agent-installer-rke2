...
To build locally the following environment varables must be set
    ...
    `$env:REPO - The dockerhub repo the image will be pushed to (this probably shouldn't be 'rancher')
"
...
if ((-not $env:REPO) -or ($env:REPO -eq "")) {
    Write-Host "`$env:REPO must be set to a valid dockerhub repository. This likely shouldn't be 'rancher'"
    exit 1
}
